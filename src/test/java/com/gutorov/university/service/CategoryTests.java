...
public class CategoryTests {
    ...
    void getEntity_WithExistingId_ReturnsCategory() {
        final var category = service.create(mapper.toRequest("Category 1"));
        assertCategoriesAreEqual(category, service.getEntity(category.getId()));
    }

    @Test
    void getAll_WithNotEmptyRepository_ReturnsCategorys() {
        service.create(mapper.toRequest("Category 1"));
        service.create(mapper.toRequest("Category 2"));
        service.create(mapper.toRequest("Category 3"));
    ...
    void create_WithValidData_ReturnCategory() {
        final var createdCategory = service.create(mapper.toRequest("Category 1"));
    ...
    void update_WithValidData_ReturnsUpdatedCategory() {
        final var createdCategoryId = service.create(mapper.toRequest("Category 1")).getId();
        final var updatedCategory = service.update(createdCategoryId, mapper.toRequest("Category 2"));
